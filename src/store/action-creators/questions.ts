...
export const fetchQuestions = (
  ...
  return async (dispatch: Dispatch<QuestionsAction>) => {
    ...
    instance
      .get<IQuestion[]>(
        `forum/questions/${params ? `${!path ? "?" : ""}${params}` : ""}`
      )
      .then((response) => {
        dispatch({
      ...
      .catch((error) => {
        dispatch({
          ...
          payload: "Error while loading questions",
        });
      });
  };
};
...
export const fetchQuestion = (
  ...
  return async (dispatch: Dispatch<QuestionsAction>) => {
    ...
    instance
      .get<IQuestion>(`forum/questions/${id}/`)
      .then((response) => {
        dispatch({
        ...
        if (addView)
          instance.patch<IQuestion>(
            `forum/questions/${id}/views/?views=${response.data.views + 1}`
          );

        instance
          .get<IUser>(`accounts/users/${response.data.author_id}/`)
          .then((response) => {
      ...
      .catch((error) => {
        dispatch({
          type: QuestionsActionTypes.FETCH_QUESTIONS_ERROR,
          payload: "Error while loading question",
        });
      });
  };
};
...
export const fetchQuestionsByTag = (tagName: string) => {
  return async (dispatch: Dispatch<QuestionsAction>) => {
    dispatch({ type: QuestionsActionTypes.FETCH_QUESTIONS });
    instance.get<ITag>(`forum/tags/${tagName}`).then((response) => {
