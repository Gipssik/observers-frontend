...
export const LoginSchema = Yup.object().shape({
  username: Yup.string()
    .matches(
      /^[a-zA-Z0-9_]+$/,
      "Username must contain only alphanumeric character plus _."
    )
...
export const AddQuestionSchema = Yup.object().shape({
  ...
  content: Yup.string()
    .test(
      "content",
      "Question's content must be at least 32 character long",
      (val) =>
        val ? val.replaceAll(/<p>(&nbsp;)*<\/p>/gi, "").length > 40 : false
    )
    .required("Required"),
  tags: Yup.string(),
});
...
export const AddCommentSchema = Yup.object().shape({
  comment: Yup.string()
    .test(
      "comment",
      "Comment's content must be at least 32 character long",
      (val) =>
        val ? val.replaceAll(/<p>(&nbsp;)*<\/p>/gi, "").length > 40 : false
    )
    .required("Required"),
...
export const AddArticleSchema = Yup.object().shape({
  ...
  content: Yup.string()
    .test(
      "content",
      "Article's content must be at least 32 character long",
      (val) =>
        val ? val.replaceAll(/<p>(&nbsp;)*<\/p>/gi, "").length > 40 : false
    )
    .required("Required"),
