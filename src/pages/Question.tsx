...
const Question: FC = () => {
  const { question, questions, loading, error } = useTypedSelector(
  ...
  const loadCommentators = async (coms: IComment[]) => {
    ...
    for (let c of coms) {
      ...
      if (s.length === 0) {
        await instance
          .get<IUser>(`accounts/users/${c.author_id}/`)
          .then((response) => {
  ...
  useEffect(() => {
    if (!questions) {
      ...
      if (q) {
        instance
          .get<IUser>(`accounts/users/${q?.author_id}/`)
          .then((response) => {
        ...
        instance.patch<IQuestion>(
          `forum/questions/${id}/views/?views=${q?.views + 1}`
        );
      }
    }

    instance
      .get<IComment[]>(`forum/comments/${id}/`)
      .then((response) => {
