...
const About: FC<AccountEditProps> = ({
  setLoading,
  setModal,
  setModalData,
}) => {
  ...
  const saveEmail = async () => {
    ...
    if (
      ...
      instance
        .patch<IUser>(`accounts/users/${user.id}/`, { email: email.value })
        .then((response) => {
  ...
  const changePassword = () => {
    ...
    if (
      newPassword &&
      (newPassword.value.length < 4 || newPassword.value.length > 19)
    ) {
    ...
    if (
      oldPassword &&
      (oldPassword.value.length < 4 || oldPassword.value.length > 19)
    ) {
    ...
    instance
      .post<IToken>("token/", data)
      .then((response) => {
        instance
          .patch<IUser>(`accounts/users/${user.id}/`, {
  ...
  return (
    <div className="account-about">
      ...
      <div className="account-about-block">
        ...
        <div className="account-change-password-title">Change password:</div>
        <div className="account-field-block">
          <div className="account-field-with-label">
            <span>
              Old password:
