...
const Comment: FC<CommentProps> = ({ comment, user }) => {
  ...
  const changeAnswerStatus = async () => {
    ...
    await instance
      .patch<IComment>(`/forum/comments/${commentState.id}/`, { is_answer })
      .then((response) => {
      ...
      >
        <div className="flex gap-3">
          ...
          <div>
            <NavLink
              to={
                self?.id === user.id ? "/account" : "/account/" + user.username
              }
            ...
            <div className="text-xs">
              {new Date(commentState?.date_created)
                .toLocaleString()
                .slice(0, -3)}
            </div>
          </div>
        ...
        <div className="comment-options">
          {self?.id === question?.author_id || self?.role.title === "Admin" ? (
            <div
              ...
              className={
                "comment-answer " +
                (commentState.is_answer ? "" : "comment-not-answer")
              }
            >
              ✔
            </div>
          ) : commentState.is_answer ? (
            <div className="text-xl text-green-700">✔</div>
          ) : null}
          {commentState.author_id === self?.id ||
          self?.role.title === "Admin" ? (
            <div
              className={
                "flex gap-3 " +
                (commentState.is_answer ? "bg-secondaryBg p-1 rounded-md" : "")
              }
            >
              <Trash
      ...
      <div>
        {editing ? (
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: commentState.content }}
          ></div>
        )}
      </div>
    </div>
