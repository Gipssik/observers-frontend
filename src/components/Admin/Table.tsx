...
const Table = <T extends object>({
  ...
  const renderColumns = (obj: T) => {
    return (
      <>
        ...
        {onDelete ? (
          <td style={{ width: "10px" }}>
            <Trash onClick={() => onDelete(obj)} width={32} height={32} />
          </td>
        ) : null}
        {Object.entries(obj).map(([key, value], index) => {
          let content;
          if (Array.isArray(value)) content = value.length;
          else if (
            typeof value === "object" &&
            !Array.isArray(value) &&
            value !== null
          ) {
            if (value.title !== undefined) content = value.title;
            else content = `{${key}}`;
  ...
  return (
    <table className="admin-table">
      <thead>
        <tr>
          {onEdit ? <th style={{ width: "10px" }}>Edit</th> : null}
          {onDelete ? <th style={{ width: "10px" }}>Del</th> : null}
      ...
      <tbody>
        {objectArray?.map((obj, index) => {
          return <tr key={index}>{renderColumns(obj)}</tr>;
        })}
      </tbody>
    </table>
  );
