...
const Questions: FC = () => {
  const { questions, loading, error } = useTypedSelector(
  ...
  const onDelete = (question: IQuestion) => {
    const onConfirm = () => {
      instance
        .delete(`forum/questions/${question.id}/`)
        .then((response) => {
    ...
    dispatch({
      ...
      payload: "Are you sure you want to delete this question?",
    });
  ...
  useEffect(() => {
    if (!questions) dispatch(fetchQuestions("order_by_date=desc"));
  }, []);
  ...
  return (
    <>
      {loading ? (
        ...
        <Table onEdit={onEdit} onDelete={onDelete} objectArray={questions} />
      ) : null}
    </>
  );
};
