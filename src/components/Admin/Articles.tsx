...
const Articles: FC = () => {
  const { articles, loading, error } = useTypedSelector(
  ...
  const onDelete = (article: IArticle) => {
    const onConfirm = () => {
      instance
        .delete(`news/articles/${article.id}/`)
        .then((response) => {
    ...
    dispatch({
      ...
      payload: "Are you sure you want to delete this article?",
    });
  ...
  return (
    <>
      {loading ? (
        ...
        <Table onEdit={onEdit} onDelete={onDelete} objectArray={articles} />
      ) : null}
    </>
  );
};
